...
+ provide the history of the acquisition/processing protocols
  (traceability);
+ be in a standard format to perform analysis with different
  toolboxes;
+ have the *Minimal Structural Dataset* to derive a self-consistent
  ...
  simulated data under different output modalities (eg, EEG, MEG,
  fMRI).
...
NOTE: the following definitions are not definitive, if you come across with a better
...
NOTE: add references to the libraries and softwares that are available for
...
arrays, althoug bound to the surface instance they were derived from. Read
...
any other similar tool like FieldTrip) to generate good forward models for
EEG/MEG)
...
* Mesh surface for the cortex (regularised, continuous and complete per
 ...
 each hemisphere should be topologically spherical):
   ...
   + triangles (triplets of indices into the vertices array, TRIANGLES -- not
...
***
Ideally, for a complete structural dataset, we should also have:            
***
...
* Basic additional information:
   + Units, ie
       - tract lengths, coordinates etc (mm).
       - strength/weights units, (au) if none.
...
https://github.com/the-virtual-brain/docs/blob/trunk/UserGuide/DataExchange.rst
...
+ The parcellation was chosen to be as homologous as possible between Macaque
  and Human. (See the [scalable brain atlas interactive tool](http://scalablebrainatlas.incf.org/main/coronal3d.php?template=PHT00&plugin=CoCoMac))
...
Atlases: In order to compare different brains, it is necessary to register them to a common space by using a template.  
